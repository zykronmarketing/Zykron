...
/* FIX: use stable statsKey string instead of stats array reference */
function useAnimatedStats(stats, active, reducedMotion) {
  const statsKey = stats.map(([v]) => v).join("|");
  ...
  useEffect(() => {
    ...
    if (reducedMotion) { setValues(parsed.map((item) => item.numeric)); setDone(true); return; }
    let raf = 0, start = 0;
    const duration = 1800;
    ...
    const tick = (time) => {
      ...
      setValues(parsed.map((item) => Math.round(item.numeric * ease(progress))));
      if (progress < 1) { raf = requestAnimationFrame(tick); }
      else { setValues(parsed.map((item) => item.numeric)); setDone(true); }
    };
    raf = requestAnimationFrame(tick);
    return () => cancelAnimationFrame(raf);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [active, reducedMotion, statsKey]);
  ...
  return stats.map(([finalValue, label], index) => {
    ...
    return [done ? `${numeric}${suffix}` : `${values[index]}`, label];
  });
}
...
const T = {
  en: {
    ...
    cta: "Let's Talk",
    heroLine1: "We Build The", heroLine2: "Future", heroLine3: "of Digital",
    ...
    heroCta1: "Start Your Project", heroCta2: "View Our Work",
    stats: [["50+","Projects Delivered"],["98%","Client Satisfaction"],["3×","Average ROI"],["24h","Response Time"]],
    scroll: "Scroll",
    svcLabel: "What We Do", svcTitle1: "Services Built", svcTitle2: "For Results",
    svcSub: "Every service we offer is engineered with a singular objective: measurable growth for your brand.",
    svcLearn: "Learn More", svcClose: "Show Less",
    portLabel: "Our Work", portTitle1: "Selected", portTitle2: "Portfolio",
    portAll: "View All Projects", portCase: "Case Study →",
    processLabel: "Process", processTitle1: "How We", processTitle2: "Work",
    ...
    testimonialsLabel: "Testimonials", testimonialsTitle1: "What Clients", testimonialsTitle2: "Say",
    testimonialsSub: "A premium digital experience only matters when it creates clarity, speed and measurable business value.",
    ctaLabel: "Get In Touch", ctaTitle1: "Ready To Build", ctaTitle2: "Something Great?",
    ...
    formTitle: "Brief your project", formName: "Name", formEmail: "E-mail", formMessage: "Message", formSubmit: "Send Message",
    formSuccessTitle: "Message sent", formSuccessText: "Thanks. We received your brief and will respond within 24 hours.",
    formErrors: { name: "Please enter your name.", email: "Please enter a valid e-mail.", message: "Please tell us a little about your project." },
    ...
    services: [
      { num:"01", title:"Web Development & AI Sites", desc:"We engineer blazing-fast, AI-powered websites built to convert. From architecture to deployment — pure performance, zero compromise.", tags:["Next.js","React","AI Integration","CMS"], deliverables:["Wireframes and interactive prototypes","AI integrations and CMS architecture","Production deployment with performance checks","Conversion tracking and launch support"] },
      { num:"02", title:"Audiovisual & High-Conversion Reels", desc:"Cinematic content engineered to stop the scroll. We produce short-form assets built for maximum retention and brand impact.", tags:["Video Production","Motion Design","Reels","Storytelling"], deliverables:["Creative direction and shot planning","Motion design and editing for retention","Platform-ready formats for Reels, Shorts and TikTok","Publishing guidance and performance review"] },
      { num:"03", title:"Social Media Growth & Marketing", desc:"Data-driven strategies that build audiences and convert followers into paying customers at scale — organically and through paid channels.", tags:["Growth Strategy","Paid Ads","Analytics","Community"], deliverables:["Audience and funnel strategy","Paid traffic campaign setup","Creative testing and analytics dashboards","Monthly optimization roadmap"] },
    ],
    process: [
  ...
  pt: {
    ...
    cta: "Fale Conosco",
    heroLine1: "Construímos", heroLine2: "O Futuro", heroLine3: "Digital com",
    ...
    heroCta1: "Começar Projeto", heroCta2: "Ver Nossos Cases",
    stats: [["50+","Projetos Entregues"],["98%","Satisfação dos Clientes"],["3×","ROI Médio"],["24h","Tempo de Resposta"]],
    scroll: "Rolar",
    svcLabel: "Como Atuamos", svcTitle1: "Nossos", svcTitle2: "Serviços",
    svcSub: "Cada entrega é orientada por um objetivo claro: crescimento real e mensurável para o seu negócio.",
    svcLearn: "Saiba Mais", svcClose: "Mostrar Menos",
    portLabel: "Cases", portTitle1: "Projetos", portTitle2: "Realizados",
    portAll: "Ver Todos os Cases", portCase: "Ver Case →",
    processLabel: "Processo", processTitle1: "Como", processTitle2: "Trabalhamos",
    ...
    testimonialsLabel: "Depoimentos", testimonialsTitle1: "O Que Dizem", testimonialsTitle2: "Os Clientes",
    ...
    ctaLabel: "Fale com a Gente", ctaTitle1: "Vamos Criar", ctaTitle2: "Algo Grande?",
    ...
    formTitle: "Conte sobre o projeto", formName: "Nome", formEmail: "E-mail", formMessage: "Mensagem", formSubmit: "Enviar Mensagem",
    formSuccessTitle: "Mensagem enviada", formSuccessText: "Obrigado. Recebemos seu briefing e responderemos em até 24 horas.",
    formErrors: { name: "Informe seu nome.", email: "Informe um e-mail válido.", message: "Conte um pouco sobre o seu projeto." },
    footerTagline: "Experiências digitais premium para marcas que avançam rápido e pensam maior.",
    footerServices: "Serviços", footerCompany: "Empresa", footerContact: "Contato", /* FIX: "Company" → "Empresa" */
    footerCompanyLinks: ["Sobre","Cases","Blog","Carreiras"],
    footerCopy: "© 2025 Zykron. Todos os direitos reservados.",
    services: [
      { num:"01", title:"Desenvolvimento Web & Sites com IA", desc:"Desenvolvemos sites ultrarrápidos e com IA, feitos para converter. Da arquitetura ao deploy — performance pura, sem concessões.", tags:["Next.js","React","Integração IA","CMS"], deliverables:["Wireframes e protótipos interativos","Integrações com IA e arquitetura CMS","Deploy em produção com checagem de performance","Métricas de conversão e suporte de lançamento"] },
      { num:"02", title:"Audiovisual & Reels de Alta Conversão", desc:"Conteúdo cinematográfico projetado para parar o scroll. Produzimos ativos de curta duração com máxima retenção e impacto de marca.", tags:["Produção de Vídeo","Motion Design","Reels","Storytelling"], deliverables:["Direção criativa e planejamento de captação","Motion design e edição focada em retenção","Formatos prontos para Reels, Shorts e TikTok","Orientação de publicação e análise de desempenho"] },
      { num:"03", title:"Crescimento em Redes Sociais & Marketing", desc:"Estratégias baseadas em dados que constroem audiências e convertem seguidores em clientes pagantes em escala — orgânico e pago.", tags:["Estratégia de Crescimento","Tráfego Pago","Analytics","Comunidade"], deliverables:["Estratégia de audiência e funil","Configuração de campanhas de tráfego pago","Testes criativos e dashboards de analytics","Roadmap mensal de otimização"] },
    ],
    process: [
...
const NAV_TARGETS = ["#home","#services","#portfolio"];
const SECTION_IDS = ["home","services","process","portfolio","testimonials","contact"];

/* ── SVG Icons ──────────────────────────────────────────────── */
const IconMonitor = () => (<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><rect x="2" y="3" width="20" height="14" rx="2"/><polyline points="8 21 12 17 16 21"/><line x1="8" y1="21" x2="16" y2="21"/></svg>);
const IconVideo  = () => (<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><polygon points="23 7 16 12 23 17 23 7"/><rect x="1" y="5" width="15" height="14" rx="2"/></svg>);
const IconTrend  = () => (<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>);
const SVC_ICONS  = [<IconMonitor key="m"/>, <IconVideo key="v"/>, <IconTrend key="t"/>];

const SocialIcon = ({ type }) => {
  const p = { width:18, height:18, viewBox:"0 0 24 24", fill:"none", stroke:"currentColor", strokeWidth:1.7, strokeLinecap:"round", strokeLinejoin:"round", "aria-hidden":true };
  if (type==="instagram") return <svg {...p}><rect x="3" y="3" width="18" height="18" rx="5"/><circle cx="12" cy="12" r="4"/><circle cx="17.5" cy="6.5" r=".8" fill="currentColor" stroke="none"/></svg>;
  if (type==="linkedin")  return <svg {...p}><path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-4 0v7h-4v-7a6 6 0 0 1 6-6z"/><rect x="2" y="9" width="4" height="12"/><circle cx="4" cy="4" r="2"/></svg>;
  if (type==="youtube")   return <svg {...p}><path d="M22 12s0-4-1-5c-.8-.9-1.7-1-2.1-1.1C15.9 5.6 12 5.6 12 5.6s-3.9 0-6.9.3C4.7 6 3.8 6.1 3 7c-1 1-1 5-1 5s0 4 1 5c.8.9 1.8 1 2.2 1.1 3 .3 6.8.3 6.8.3s3.9 0 6.9-.3c.4-.1 1.3-.2 2.1-1.1 1-1 1-5 1-5z"/><polygon points="10 9 16 12 10 15 10 9" fill="currentColor" stroke="none"/></svg>;
  return <svg {...p}><path d="M4 4l16 16"/><path d="M20 4L4 20"/></svg>;
};
...
const GLOBAL_CSS = `
  *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
  ...
  a:focus-visible, button:focus-visible, input:focus-visible, textarea:focus-visible {
    outline:2px solid #0047FF; outline-offset:3px;
  }
  .font-bebas { font-family:'Bebas Neue',sans-serif; }
  .font-syne  { font-family:'Syne',sans-serif; }
  .font-dm    { font-family:'DM Sans',sans-serif; }
  .content-wrap { transition:opacity 150ms ease; }
  .content-wrap.fading { opacity:.2; }
  .hero-grid {
    background-image:
  ...
  .nav-lnk::after { content:''; position:absolute; bottom:-3px; left:0; width:0; height:1px; background:#0047FF; transition:width .3s; }
  .nav-lnk:hover::after, .nav-lnk.active::after { width:100%; }
  .nav-lnk:hover, .nav-lnk.active { color:#fff !important; }
  .btn-primary { position:relative; overflow:hidden; background:#0047FF; transition:transform .25s,box-shadow .25s; }
  .btn-primary::after { content:''; position:absolute; inset:0; background:linear-gradient(135deg,rgba(255,255,255,.15) 0%,transparent 55%); opacity:0; transition:opacity .3s; }
  ...
  .hamburger { display:none; width:42px; height:42px; border:1px solid rgba(255,255,255,.14); border-radius:10px; background:rgba(255,255,255,.04); color:#fff; align-items:center; justify-content:center; cursor:pointer; }
  ...
  .mobile-menu { position:fixed; inset:64px 0 0; z-index:190; background:rgba(0,0,0,.96); backdrop-filter:blur(22px) saturate(180%); transform:translateY(-12px); opacity:0; pointer-events:none; transition:opacity .25s,transform .25s; display:flex; flex-direction:column; align-items:center; justify-content:center; gap:2rem; }
  ...
  .svc-card { border:1px solid rgba(255,255,255,.07); background:linear-gradient(145deg,rgba(8,8,20,1) 0%,#000 100%); transition:transform .4s cubic-bezier(.23,1,.32,1),border-color .4s,box-shadow .4s; position:relative; overflow:hidden; }
  .svc-card::before { content:''; position:absolute; top:0; left:0; right:0; height:2px; background:linear-gradient(90deg,transparent,#0047FF,transparent); transform:scaleX(0); transition:transform .5s; }
  .svc-card::after { content:''; position:absolute; inset:0; background:radial-gradient(ellipse 80% 50% at 50% 100%,rgba(0,71,255,.07) 0%,transparent 70%); opacity:0; transition:opacity .4s; pointer-events:none; }
  .svc-card:hover::before, .svc-card.expanded::before { transform:scaleX(1); }
  .svc-card:hover::after,  .svc-card.expanded::after  { opacity:1; }
  .svc-card:hover, .svc-card.expanded { transform:translateY(-8px) scale(1.012); border-color:rgba(0,71,255,.45); box-shadow:0 24px 64px rgba(0,71,255,.13),0 0 40px rgba(0,71,255,.08),inset 0 0 40px rgba(0,71,255,.04); }
  .svc-icon { transition:background .35s,box-shadow .35s,transform .35s; }
  .svc-card:hover .svc-icon, .svc-card.expanded .svc-icon { background:rgba(0,71,255,.2) !important; box-shadow:0 0 24px rgba(0,71,255,.5); transform:scale(1.05) rotate(-3deg); }
  .svc-more { max-height:0; opacity:0; overflow:hidden; transition:max-height .45s cubic-bezier(.23,1,.32,1),opacity .3s; }
  .svc-more.open { max-height:260px; opacity:1; }
  .svc-arrow { transition:transform .25s; display:inline-block; }
  .svc-arrow.open { transform:rotate(180deg); }
  .process-card { position:relative; border:1px solid rgba(255,255,255,.07); background:rgba(255,255,255,.025); border-radius:16px; padding:2rem; transition:border-color .3s,transform .3s,background .3s; }
  ...
  .process-pulse { width:10px; height:10px; border-radius:50%; background:#0047FF; box-shadow:0 0 0 0 rgba(0,71,255,.7); display:inline-block; flex-shrink:0; }
  .port-card { position:relative; overflow:hidden; cursor:pointer; border:1px solid rgba(255,255,255,.06); transition:border-color .4s,box-shadow .4s; border-radius:14px; }
  ...
  .port-overlay { position:absolute; inset:0; background:linear-gradient(175deg,rgba(0,20,150,.92) 0%,rgba(0,0,8,.9) 100%); opacity:0; transition:opacity .4s cubic-bezier(.23,1,.32,1); display:flex; flex-direction:column; justify-content:flex-end; padding:1.75rem; }
  ...
  .portfolio-cursor { position:fixed; width:48px; height:48px; border-radius:50%; pointer-events:none; z-index:220; transform:translate(-50%,-50%); background:rgba(0,71,255,.36); border:1px solid rgba(120,160,255,.65); mix-blend-mode:screen; opacity:0; transition:opacity .18s; }
  .portfolio-cursor.show { opacity:1; }
  .testimonial-card { border:1px solid rgba(255,255,255,.07); border-radius:18px; background:linear-gradient(145deg,rgba(255,255,255,.045),rgba(0,0,0,.65)); box-shadow:0 28px 90px rgba(0,71,255,.08); overflow:hidden; }
  /* FIX: testimonial fade transition */
  .testimonial-slide { transition:opacity .3s ease, transform .3s ease; }
  .testimonial-slide.hidden { opacity:0; transform:translateY(10px); pointer-events:none; }
  ...
  .form-input { width:100%; border:1px solid rgba(255,255,255,.1); background:rgba(255,255,255,.04); color:#fff; border-radius:10px; padding:.9rem 1rem; font-family:'DM Sans',sans-serif; font-size:14px; transition:border-color .25s,box-shadow .25s,background .25s; }
  ...
  .form-error { min-height:16px; color:#8FB0FF; font-size:12px; font-family:'DM Sans',sans-serif; }
  ...
  .stat-val { background:linear-gradient(120deg,#fff 20%,#0047FF 60%,#fff 80%); background-size:200% auto; -webkit-background-clip:text; -webkit-text-fill-color:transparent; background-clip:text; }
...
export default function ZykronLanding() {
  ...
  const [hPort, setHPort] = useState(null);
  ...
  const [statsRef, statsIn] = useInView(0.35);
  const [svcRef,   svcIn]   = useInView();
  const [processRef, processIn] = useInView();
  const [portRef,  portIn]  = useInView();
  ...
  /* Scroll events */
  useEffect(() => {
    const fn = () => { setScrolled(window.scrollY > 50); if (window.scrollY > 8) setHideScrollCue(true); };
    fn();
  ...
  /* Typewriter reset on lang change */
  useEffect(() => {
    setWordIdx(0);
  ...
  }, [charIdx, deleting, wordIdx, lang, reducedMotion]);

  /* Testimonial auto-rotate */
  useEffect(() => {
    if (reducedMotion) return;
    const id = setInterval(() => changeTestimonial(i => (i+1) % t.testimonials.length), 5000);
    return () => clearInterval(id);
  }, [reducedMotion, t.testimonials.length]);

  /* FIX: testimonial fade transition */
  const changeTestimonial = (fn) => {
    setTestimonialVisible(false);
    setTimeout(() => {
      setTestimonialIdx(fn);
  ...
  const changeLang = (next) => {
    ...
    setTimeout(() => { setLang(next); setContentFading(false); }, 150);
  };
  ...
  const submitForm = (e) => { e.preventDefault(); if (validateForm()) setFormSent(true); };
  const updateField = (field, value) => { setFormData(c => ({ ...c, [field]:value })); setFormErrors(c => ({ ...c, [field]:undefined })); };
  ...
  return (
    <div style={{ background:"#000", color:"#fff", fontFamily:"'DM Sans',sans-serif", overflowX:"hidden" }}>
      ...
      {/* ── NAV ─────────────────────────────────────────────── */}
      <nav style={{ position:"fixed", top:0, left:0, right:0, zIndex:200, padding:"0 2.5rem", height:64, display:"flex", alignItems:"center", justifyContent:"space-between", transition:"background .4s,border-color .4s,backdrop-filter .4s", backdropFilter:scrolled||mobileOpen?"blur(24px) saturate(180%)":"none", background:scrolled||mobileOpen?"rgba(0,0,0,.78)":"transparent", borderBottom:scrolled||mobileOpen?"1px solid rgba(255,255,255,.07)":"1px solid transparent" }}>
        <a href="#home" onClick={() => setMobileOpen(false)} aria-label="Zykron home" style={{ display:"flex", alignItems:"center", gap:10, textDecoration:"none" }}>
          <ZLogo size={34}/><span className="font-syne" style={{ fontWeight:800, fontSize:18, letterSpacing:"-.02em", color:"#fff" }}>Zykron</span>
        </a>
        <div className="nav-links" style={{ display:"flex", alignItems:"center", gap:"2.5rem" }}>
          {t.nav.map((lbl, i) => (
            <a key={lbl} href={NAV_TARGETS[i]} onClick={() => setMobileOpen(false)}
              className={`nav-lnk font-syne${activeSection === NAV_TARGETS[i].replace("#","") ? " active" : ""}`}
              style={{ color:activeSection===NAV_TARGETS[i].replace("#","")?"#fff":"rgba(255,255,255,.68)", textDecoration:"none", fontSize:13, fontWeight:600, letterSpacing:".06em", textTransform:"uppercase", transition:"color .25s" }}>
              {lbl}
            </a>
          ))}
        </div>
        <div style={{ display:"flex", alignItems:"center", gap:12 }}>
          ...
          <a href="#contact" onClick={() => setMobileOpen(false)} className="btn-primary nav-cta font-syne" style={{ color:"#fff", textDecoration:"none", padding:".55rem 1.4rem", borderRadius:7, fontSize:13, fontWeight:700, letterSpacing:".05em", display:"flex", alignItems:"center", gap:6 }}>
            {t.cta}<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" aria-hidden="true"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
          </a>
      ...
      <div className={`mobile-menu${mobileOpen?" open":""}`} aria-hidden={!mobileOpen}>
        {t.nav.map((lbl, i) => (<a key={lbl} href={NAV_TARGETS[i]} onClick={() => setMobileOpen(false)} className="font-bebas">{lbl}</a>))}
        <a href="#contact" onClick={() => setMobileOpen(false)} className="font-bebas">{t.cta}</a>
      </div>
      ...
      <div className={`content-wrap${contentFading?" fading":""}`}>
        ...
        <section id="home" style={{ minHeight:"100vh", position:"relative", display:"flex", alignItems:"center", justifyContent:"center", overflow:"hidden", paddingTop:64 }}>
          ...
          <div className="noise"/><div className="scanline"/>
          {[[6,15,80],[94,25,70]].map(([l,t2,b],i) => (
            <div key={i} style={{ position:"absolute", left:`${l}%`, top:`${t2}%`, bottom:`${100-t2-b}%`, width:1, background:`linear-gradient(to bottom,transparent,rgba(0,71,255,${i===0?.45:.25}),transparent)`, pointerEvents:"none" }}/>
          ))}
          ...
          <div style={{ textAlign:"center", maxWidth:960, padding:"0 1.5rem", position:"relative", zIndex:2 }}>
            ...
            <p className="font-dm hero-fade-2" style={{ fontSize:"clamp(.95rem,2vw,1.1rem)", color:"rgba(255,255,255,.62)", maxWidth:520, margin:"0 auto 2.8rem", lineHeight:1.75 }}>{t.heroSub}</p>
            <div className="hero-btns hero-fade-3" style={{ display:"flex", gap:"1rem", justifyContent:"center", flexWrap:"wrap" }}>
              <a href="#contact" className="btn-primary font-syne" style={{ color:"#fff", textDecoration:"none", padding:".9rem 2.4rem", borderRadius:8, fontSize:14, fontWeight:700, letterSpacing:".06em", display:"flex", alignItems:"center", gap:8 }}>
                {t.heroCta1}<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" aria-hidden="true"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
              </a>
        ...
        <section id="testimonials" style={{ padding:"7rem 2rem 8rem" }}>
          <div style={{ maxWidth:980, margin:"0 auto" }}>
            ...
            <div className={`testimonial-card reveal${testIn?" show":""} d1`}>
              <div className={`testimonial-slide${testimonialVisible?"":" hidden"}`} style={{ padding:"2.3rem" }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", gap:"1rem", marginBottom:"2rem" }}>
                  <button type="button" aria-label="Depoimento anterior" onClick={() => changeTestimonial(i => (i-1+t.testimonials.length)%t.testimonials.length)} className="btn-ghost" style={{ width:42, height:42, borderRadius:"50%", border:"1px solid rgba(255,255,255,.18)", background:"rgba(255,255,255,.03)", color:"#fff", fontSize:28, lineHeight:1, cursor:"pointer" }}>‹</button>
                  <div style={{ display:"flex", alignItems:"center", gap:"1rem" }}>
                    <div style={{ width:62, height:62, borderRadius:"50%", display:"grid", placeItems:"center", background:"linear-gradient(145deg,#0047FF,#00164F)", boxShadow:"0 0 36px rgba(0,71,255,.32)" }}><span className="font-syne" style={{ fontWeight:800 }}>{t.testimonials[testimonialIdx].initials}</span></div>
                    <div><div className="font-syne" style={{ fontWeight:800, marginBottom:".25rem" }}>{t.testimonials[testimonialIdx].name}</div><div className="font-dm" style={{ color:"rgba(255,255,255,.55)", fontSize:13 }}>{t.testimonials[testimonialIdx].role}</div></div>
                  </div>
                  <button type="button" aria-label="Próximo depoimento" onClick={() => changeTestimonial(i => (i+1)%t.testimonials.length)} className="btn-ghost" style={{ width:42, height:42, borderRadius:"50%", border:"1px solid rgba(255,255,255,.18)", background:"rgba(255,255,255,.03)", color:"#fff", fontSize:28, lineHeight:1, cursor:"pointer" }}>›</button>
                </div>
                <div aria-label="5 stars" style={{ color:"#6D93FF", letterSpacing:".16em", marginBottom:"1.2rem", textAlign:"center" }}>★★★★★</div>
                <p className="font-dm" style={{ color:"rgba(255,255,255,.72)", fontSize:"clamp(1rem,2vw,1.2rem)", lineHeight:1.75, textAlign:"center", maxWidth:720, margin:"0 auto" }}>"{t.testimonials[testimonialIdx].text}"</p>
                <div style={{ display:"flex", justifyContent:"center", gap:".7rem", marginTop:"2rem" }}>
                  {t.testimonials.map((_, idx) => (
                    <button key={idx} type="button" aria-label={`Ver depoimento ${idx+1}`} className={`testimonial-dot${testimonialIdx===idx?" active":""}`} onClick={() => changeTestimonial(() => idx)}/>
                  ))}
                </div>
              </div>
            </div>
                ...
                {formSent ? (
                  ...
                  <form onSubmit={submitForm} noValidate>
                    ...
                    <div style={{ display:"grid", gap:".9rem" }}>
                      {[["name",t.formName,"text"],["email",t.formEmail,"email"]].map(([field, label, type]) => (
                        <label key={field} className="form-field">
                          <span className="font-dm" style={{ color:"rgba(255,255,255,.62)", fontSize:13 }}>{label}</span>
                          <input className="form-input" type={type} value={formData[field]} onChange={e => updateField(field, e.target.value)} aria-invalid={!!formErrors[field]} aria-describedby={`${field}-error`}/>
                          <span id={`${field}-error`} className="form-error">{formErrors[field]}</span>
                        </label>
                      ))}
                      <label className="form-field">
                        <span className="font-dm" style={{ color:"rgba(255,255,255,.62)", fontSize:13 }}>{t.formMessage}</span>
                        <textarea className="form-input" rows={4} value={formData.message} onChange={e => updateField("message", e.target.value)} aria-invalid={!!formErrors.message} aria-describedby="message-error"/>
                        <span id="message-error" className="form-error">{formErrors.message}</span>
                      </label>
        ...
        <footer style={{ borderTop:"1px solid rgba(255,255,255,.06)", padding:"4rem 2rem 1.6rem", background:"#000" }}>
          <div style={{ maxWidth:1240, margin:"0 auto" }}>
            <div className="footer-grid" style={{ display:"grid", gridTemplateColumns:"1.35fr 1fr 1fr 1fr", gap:"2.2rem", paddingBottom:"3rem" }}>
              <div>
                ...
                <div style={{ display:"flex", gap:".7rem" }}>
                  {["instagram","linkedin","x","youtube"].map(s => (<a key={s} href="#" aria-label={s} className="social-link"><SocialIcon type={s}/></a>))}
                </div>
              </div>
              <FooterCol title={t.footerServices} links={t.services.map(s => s.title)}/>
              <FooterCol title={t.footerCompany} links={t.footerCompanyLinks}/>
              <div>
                ...
                <div style={{ display:"grid", gap:".75rem" }}>
                  {t.ctaInfo.map(([icon, label]) => (
                    <a key={label} href={String(label).includes("hello")?"mailto:hello@zykron.io":"#contact"} className="ft-lnk font-dm" style={{ textDecoration:"none", fontSize:14, display:"flex", gap:8, alignItems:"center" }}><span>{icon}</span><span>{label}</span></a>
                  ))}
            ...
            <div className="footer-bottom" style={{ borderTop:"1px solid rgba(255,255,255,.06)", paddingTop:"1.4rem", display:"flex", justifyContent:"center", alignItems:"center", gap:"1rem" }}>
              <p className="font-dm" style={{ fontSize:12, color:"rgba(255,255,255,.48)", textAlign:"center" }}>{t.footerCopy}</p>
              <div style={{ marginLeft:"auto" }}><LangToggle lang={lang} onChange={changeLang}/></div>
            </div>
          </div>
        </footer>
      </div>
...
function LangToggle({ lang, onChange }) {
  return (
    <div className="lang-toggle">
      {["en","pt"].map(l => (
        <button key={l} type="button" aria-label={`Switch to ${l==="en"?"English":"Portuguese"}`} className={`lang-btn${lang===l?" active":""}`} onClick={() => onChange(l)}>{l==="en"?"EN":"PT"}</button>
      ))}
    </div>
  );
}
...
function ServiceCard({ svc, icon, index, expanded, hovered, inView, labelMore, labelClose, onHover, onToggle }) {
  return (
    <div className={`svc-card reveal${inView?" show":""} d${index+1}${expanded?" expanded":""}`} onMouseEnter={() => onHover(index)} onMouseLeave={() => onHover(null)} style={{ borderRadius:16, padding:"2.5rem", cursor:"default" }}>
      <div style={{ position:"relative", zIndex:1 }}>
        ...
        <button type="button" onClick={onToggle} aria-expanded={expanded} className="font-syne" style={{ border:"none", background:"transparent", padding:0, display:"flex", alignItems:"center", gap:8, fontSize:13, fontWeight:700, letterSpacing:".05em", color:hovered||expanded?"#0047FF":"rgba(255,255,255,.55)", transition:"color .3s", cursor:"pointer" }}>
          ...
          <span className={`svc-arrow${expanded?" open":""}`} aria-hidden="true">
...
function PortfolioArt({ color }) {
  ...
  return (
    <svg aria-hidden="true" viewBox="0 0 420 260" style={{ position:"absolute", inset:0, width:"100%", height:"100%", opacity:.15, color, pointerEvents:"none" }}>
      <defs><pattern id={id} width="28" height="28" patternUnits="userSpaceOnUse"><path d="M28 0H0V28" fill="none" stroke="currentColor" strokeWidth="1"/></pattern></defs>
...
function PortCard({ item, ps, hover, onEnter, onLeave, cls, gridArea, height, headingSize="1.25rem", isBanner=false, t }) {
  return (
      ...
      <div className="port-bg" style={{ width:"100%", height:"100%", background:`radial-gradient(ellipse 110% 110% at 20% 20%,${ps.accent} 0%,${ps.accentB} 40%,${ps.bg} 100%)`, position:"relative", display:"flex", flexDirection:"column", justifyContent:isBanner?"center":"space-between", padding:isBanner?"0 2.5rem":"1.75rem" }}>
        ...
        {isBanner ? (
          <div className="port-banner-inner" style={{ display:"flex", alignItems:"center", justifyContent:"space-between", position:"relative", zIndex:1, gap:"2rem" }}>
            ...
            <span className="font-syne" style={{ display:"inline-flex", alignItems:"center", gap:6, fontSize:12, fontWeight:700, letterSpacing:".07em", whiteSpace:"nowrap", border:`1px solid ${hover?"rgba(0,71,255,.6)":"rgba(255,255,255,.2)"}`, padding:".45rem 1.1rem", borderRadius:6, color:hover?"#6D93FF":"rgba(255,255,255,.78)", flexShrink:0, transition:"border-color .3s,color .3s" }}>{t.portCase}</span>
          </div>
        ) : (
          <>
            <div style={{ position:"relative", zIndex:1 }}>
